// src/HistoryModal.tsx
import { useMemo, useState } from "react";
import type { Participant } from "./types";            // cambia a "../types" si lo tienes en otra carpeta
import { computeProgress, trendLabel } from "./lib/utils"; // cambia a "../lib/utils" si aplica
import {
    LineChart,
    Line,
    XAxis,
...
export default function HistoryModal({
    ...
    const chartData = useMemo(
        () => entries.map((e: { date: string; weight: number }) => ({ date: e.date.slice(5), weight: e.weight })),
        [entries]
    );
    ...
    function exportCSV() {
        const rows: (string | number)[][] = [
            ...
            ...entries.map((e: { date: string; weight: number }) => [participant.id, participant.name, e.date, String(e.weight)]),
        ];
            ...
            <div className="bg-white rounded-2xl shadow-xl w-[95vw] max-w-3xl p-5">
                ...
                <div className="overflow-x-auto">
                    <table className="w-full text-sm">
                        ...
                        <tbody>
                            {entries.map((e: { date: string; weight: number }) => (
                            ...
                            {entries.length === 0 && (
                                <tr>
                                    <td className="py-4 text-center text-slate-500" colSpan={2}>Sin registros en el rango seleccionado</td>
                                </tr>
                            )}
                        </tbody>
                    </table>
